...
def process(input_param):
    ...
    write.csv(outTab, "{gwas_id}_outcome.csv", row.names=FALSE)
    ...
    # 此处存疑
    write.csv(outTab, "{gwas_id}_exposure.csv", row.names=FALSE)
    """
    ...
    shutil.copy(os.path.join(working_directory, f"{gwas_id}_exposure.csv"),
                os.path.join("./purified_data", f"{gwas_id}_exposure.csv"))
    shutil.copy(os.path.join(working_directory, f"{gwas_id}_outcome.csv"),
                os.path.join("./purified_data", f"{gwas_id}_outcome.csv"))
    # import time
    # time.sleep(30)

    if check_purified(gwas_id):
        print(f"PURIFICATION Data purified, result saved to ./purified_data/{gwas_id}")
